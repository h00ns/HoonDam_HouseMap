  ...
  <b-container fluid id="main" style="padding:0;">
    ...
    <div v-show="showDetail" style="width:600px; height:600px; background-color:white; position:fixed; top:80px; left:30%; z-index:100; border-radius:30px;">
         <div style="margin-top:10px; margin-left:20px; text-align:left;">
            ...
            <b-button pill v-b-toggle.sidebar-2 style="float:right; margin-right:60px;" @click="setPrice(detail)">평당가</b-button>
              <b-sidebar id="sidebar-2" title="평당가" shadow>
                      <BarChart v-if="AreaPriceSelected" id="canvas" :data="dataChart" :options="{responsive: true, maintainAspectRatio: false}"/>
              </b-sidebar>
         </div>
    ...
    <!-- 인기매물 top5 -->
    <div>
      <b-button pill variant="success" v-b-toggle.sidebar-1 id="good">인기매물</b-button>
      <b-sidebar id="sidebar-1" title="인기매물 TOP5" shadow>
        <div class="px-3 py-2">
          <div class="apt" @dblclick="move(item)" style="cursor:pointer; margin:10px; height:120px; box-shadow: 5px 5px 5px black; border-radius:30px; border:2px solid black;" v-for="(item,idx) in top" :key="idx" >
            <b-row>
                <b-col class="mt-2" style="margin-left:18px;" cols="3">
                    <img src="@/assets/HoonDam_Logo.png" style="display:inline-block; height:100px; width:100px;">
                </b-col>
                <b-col class="mt-3">
                    ...
                    <div style="position:relative; top:10px; left:10px;">
                      <img @click="modal(item)" src="@/assets/search.png" style="cursor:pointer; width:25px;" />
                      <img v-if="mydeals.findIndex(element => element.no == item.no) == -1" src="@/assets/heart.png" @click="zzim(item)" style="cursor:pointer; margin-left:2px; width:25px;"/>
                      <img v-if="mydeals.findIndex(element => element.no == item.no) != -1" src="@/assets/heartRed.png" @click="zzimNo(item)" style="cursor:pointer; margin-left:2px; width:25px;"/>
                    </div>
                </b-col>  
            </b-row>
        </div>
    ...
    <b-modal id="instaModal" ref="instaModal">
      <div>
        <img v-for="image in instaImgs" :src="image.media_url" :key="image.id" style="width:200px; height:200px; margin:10px;" :title="image.caption">
      </div>
    </b-modal>
  </b-container>
</template>
...
<script type="text/javascript" src="http://dapi.kakao.com/v2/maps/sdk.js?appkey=e2ab64b6e350b14e00ec1b5de3262fe8&libraries=services,clusterer,drawing"></script>
<script>
...
export default {
    ...
    methods: {
       ...
       move(data){
        ...
        modal(item){
          ...
          .catch(()=>{
           ...
           http
          .get(`/housedeal/count/${item.no}`)
          .then(({data})=>{
            this.$store.dispatch('countHeart',data);
          })
          .catch(()=>{
            alert('에러 발생')
          });
       ...
       // 평단가
        setPrice(detail){
                    this.AreaPriceSelected = false

          console.log(detail)
          let val = parseInt(detail.dealamount) / detail.area * 3.3 * 1000 
          console.log(val)
          this.dataChart = [0,3427, 0 , val, 0]
          console.log(this.dataChart)
          this.AreaPriceSelected = true
        }
    },
    computed : {
      deals(){
     ...
     created(){
      ...
      .then( ({data}) => {
        ...
        script.src = 'http://dapi.kakao.com/v2/maps/sdk.js?autoload=false&appkey=e2ab64b6e350b14e00ec1b5de3262fe8&libraries=services,clusterer,drawing'
